...
- 🔴 **「一次什么都没做的失败」不配用模态弹窗**（2026-09-22，构建 201 真机日志）：
  中止路径**一条记录都没删**、列表也已从本地库刷新过 ⇒ 用户**没有**需要立刻处理的后果，
  而模态窗会打断他手上正在做的事 ⇒ **不弹**，只写日志（级别仍按失败形态分）。
  ⚠️ 反例（本项目为此踩了两次）：中止时弹一个 `SEAL-INSTALL-707`，而
  `performAlertRecovery` → `settingsRoute` 对 **`SEAL-INSTALL-` 前缀一律路由到
  `.localDevVPN`** ⇒ 用户每下拉一次刷新就被送进 VPN 设置页（页面上那个按钮正是
  「检测当前 VPN…」），而其实**什么都没坏** ✗。
  **判据：模态窗只留给「用户必须马上知道、否则会做错事」的失败**；
  「VPN 没开」有自己的入口（设置里的通道检测写 `SEAL-INSTALL-706b`）。
- 🔴 **「撞满超时 = 会话已死」也是错的**（同一份日志）：12:46:59 超时（15.0 秒）→ 12:47:37
  通道检测又「正常」；12:47:51 超时（15.1 秒）→ 12:47:55 对账正常跑完（还删了 1 条）
  ⇒ 15 秒超时是**会话暂时卡住**，不是死。
  ⇒ **不要拿「耗时」当「要不要打断用户」的判据**；耗时只用来分**日志级别**
  （`InstalledAppProbeFailurePolicy`，与重试判据共用同一个阈值）。
- 🔴 **破坏性操作必须留痕到「删了谁」**：`对账完成：探测 N 条，删除 M 条` **不足以事后核对**
  —— 真机日志里出现「探测 2 条，删除 1 条」，而那 2 条正是用户 9 分钟前与 1.5 分钟前
  **刚装成功**的 App ⇒ **删的是谁、设备怎么答的，日志里查不出来** ✗。
  ⇒ 每次删除单独一条日志（App 名 ＋ Bundle ID ＋ 设备的回答 ＋ 触发来源），汇总行再列名字。
  **判据：拿着日志要能说出「删了谁、为什么删、谁触发的」。**
- 🔴 **删除前必须「再问一次」**（同一条日志）：**阳性对照通过只证明通道整体是通的**，
  证明不了「它对**这一个** Bundle ID 的否定答案是对的」——
  底层把 lookup 的 `Err` 与「没查到」折成**同一个空指针** ⇒ **单条**查询失败也会伪装成「没装」✗。
  ⇒ 设备答「未安装」之后**再问一次**，两次都是「未安装」才真的删
  （判据 = 纯函数 `InstalledAppReconcilePolicy.confirmedRemoval`，有单测）；
  不一致（或第二次问不通）就**中止整轮**、不删任何东西（`SEAL-RECONCILE-008`，
  日志里写出两次分别答了什么）。
  **判据：阳性对照覆盖的是那个折叠陷阱的「全局」形态，单条形态只能靠「问两次」。**
- ℹ️ **触发来源必须写进日志**（下拉刷新 vs 启动/回到前台）：这条链路有 3 个触发点，
  而排查时第一个要问的就是「用户当时是不是主动下拉了」——
  构建 201 的日志里有 2 条超时中止，正因为缺这一项而**无法判断用户会不会看到弹窗**。
- 🔴 **破坏性操作的「失败」必须让用户看见 —— 静默 `return false` ＋ 调用点丢弃返回值
  = 点了没反应**（2026-09-22，用户报「续签和待签名删不掉」）。
  **两个条件同时成立才会出事**，所以两边都要查：
  ① **被调用方**：`delete(_:)` 开头两处 `guard ... else { return false }`
  （`appStore`/`fileStore` 为空、租约被占用）**不写日志、不设 `alertFailure`**；
  ② **调用方**：`Task { _ = await viewModel.delete(app) }` —— **返回值被丢掉了** ✗
  ⇒ 用户点完「删除」，App 还在、**一句话都没有**；日志里也**一条相关记录都没有**
  （构建 199 的 819 行日志里删除相关为**零**，这就是定位它的唯一线索）。
  ⚠️ 租约那处更糟：`OperationCoordinator` 是**单槽全局租约**，`beginWaiting` 会
  **先等满 30 秒**再返回 nil ⇒ 界面毫无反应地卡半分钟，然后依然什么都没发生。
  ⇒ **判据：`delete` 的每一条失败出口都必须设置 `alertFailure` ＋ 写日志码。**
  退化成一行式 `guard ... else { return false }` 时守卫会红（R76）。
  ⚠️ **同族自查**：任何「返回 `Bool` 但调用点用 `_ =` 丢掉」的破坏性操作都有这个形态 ——
  先 `grep "_ = await"` 把调用点列出来。
- 🔴 **用户主动的破坏性操作成功也要留痕**（同一条）：日志必须能区分
  「**他自己删的**」（`SEAL-APP-006`）与「**对账删的**」（`SEAL-RECONCILE-007`）——
  用户报「我的 App 不见了」时，这一条决定你能不能回答他。
  ⇒ 失败码 `SEAL-APP-004`（租约被占用，**提示里要说出是谁占着**）/ `SEAL-APP-005`（存储未就绪）。
...
- 🔴 **第②类「做了事但没做成」的日志必须带底层原因**（`domain` + `code` + 描述）。
  判据：**拿着这条日志，能不能直接说出下一步该做什么？** 说不出就是缺原因。
  反面同样成立：**「用户没给权限 / 开关关着」这类已知条件不属于失败**（第③类
  「条件不满足 ⇒ 跳过」），别记成 error —— 它会把真问题淹掉。
  ⚠️ 2026-09-22 实测：`SEAL-NOTIFY-002a` 以 error 出现 **100 次**，每次都只有
  「通知调度失败」五个字，而触发点是**每次刷新都跑一遍的后台重排**
  ⇒ 一次刷屏 + 完全无法归因。判据抽成纯函数（`ExpiryNotificationSchedulingPolicy`）+ 单测。
- ⚠️ **别让「暂时的读不到」变成模态弹窗 + 跳转**：`alertFailure` 的按钮文案只要不是
  「知道了」，`performAlertRecovery` 就会按 `settingsRoute` 把 `SEAL-INSTALL-` 前缀
  **一律路由到 LocalDevVPN 设置页**。一次**什么都没改动**的失败（如对账中止）把用户
  推进 VPN 页面就是假警报（2026-09-22 用户原话：「有时候刷新还是出现弹窗让去检测VPN」）。
  做法分两层，**缺一层都不够**：
  ① **有界重试**（判据 `InstalledAppProbeRetryPolicy`：只重试快速失败、且有次数上限）——
  它只提高成功率，**消不掉**假警报（真机实测重试窗口 ~2.2 秒 < 通道恢复 ~3 秒）；
  ② 🔴 **换判据**（判据 `InstalledAppProbeFailurePolicy`）：**只有「撞满超时 = 会话已死」
  才允许打断用户**；「快速失败 = 通道还没起来」必须**静默**（本轮什么都没改 ⇒ 第③类
  「条件不满足 ⇒ 跳过」，连日志级别都该是 info 而不是 warning）。
  ⇒ **判据：一次「什么都没做」的失败，值不值得让用户离开当前界面？** 答案是否时，
  就不要用会跳转的弹窗；而「要不要打扰用户」与「日志级别」必须**共用同一个纯判据**，
  分两处写会出现「日志说没事、弹窗却弹出来」。
  仍要弹窗时说明「本轮未改动任何记录」，按钮用「知道了」。
