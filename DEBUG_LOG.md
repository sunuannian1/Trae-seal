...
- 🔴 **用户报「续签失败」时，先查通道，不要先读签名代码**（2026-09-22，构建 199 日志实证）。
  一次「续签完成：共 3，成功 0，失败 3」看着像签名问题，实际**与签名毫无关系**：
  - **数一遍通道检测的失败原因**：58 次检测里 35 次失败，首个错误**全部**是
    「设备未响应」（`SEAL-INSTALL-708`）×**24** / 「LocalDevVPN 未就绪」（`SEAL-INSTALL-701`）×**11**
    ⇒ 没有一次是证书 / App ID / 描述文件 / 门户。
  - **同一批日志里的签名链路全程成功**（全是信息级、无一条错误）：
    `证书决策：复用 Apple 生效列表中的本机证书…剩余有效期已通过完整 7 天校验` →
    `描述文件核验：Bundle=…，UUID=…` → `签名：重签完成（逐 Mach-O 串行）` →
    `签名：打包完成（deflate）` → `签名产物核验通过`。
  - **结论**：续签 = 签名 ＋ 安装，而失败只发生在**安装**那一步 ⇒ 根因是
    **设备通道不通（LocalDevVPN 隧道）**，与 iOS 16 配对卡住的是同一件事。
  - **判据**：看到续签失败，先 `grep -c "首个错误"` 把**通道**的失败原因数出来；
    只有当通道正常、失败发生在签名阶段时才去读签名代码。
- ⚠️ **通道检测不能并发（同步 FFI 不可取消），反复点只会互相排队、越点越慢**（同一份日志）。
  实测：**58 次「通道检测开始」**，其中 **4 条在同一秒（`12:57:50`）一起结束**
  （⇒ 当时确实有多个检测同时在跑），还有一条从 `11:56:44` 一直跑到 `12:55`
  （**3522 秒 ≈ 59 分钟**）。正常一次约 **20 秒**（36 轮 × ~0.5 秒）。
  ⇒ 验收单必须写「**只触发一次、耐心等**」；想放弃就离开界面，不要靠再点一次「催」。
- 🔴 **「静默 `return false`」＋「调用点丢弃返回值」= 用户点了没反应**（2026-09-22，用户报
  「续签和待签名删不掉」）。**两个条件同时成立才会出事**，所以查这类报障要**两头都看**。
  - **症状**：用户点「删除」，App 还在、界面**一句话都没有**；而导出的日志里
    **一条删除相关的记录都没有**（构建 199 的 819 行日志，删除相关为**零**）——
    这个「零」就是定位它的唯一线索：不是「删失败」，是**这条路径从不留痕**。
  - **根因（两处叠加）**：① `AppsViewModel.delete(_:)` 开头两处
    `guard ... else { return false }`（`appStore`/`fileStore` 为空、操作租约被占用）
    **既不写日志、也不设 `alertFailure`**；② 调用点
    `AppsViewRoot.swift:121` 是 `Task { _ = await viewModel.delete(app) }`
    —— **返回值被 `_ =` 丢掉了** ✗ ⇒ 没有任何一层会告诉用户「没删成」。
  - **为什么租约那处特别糟**：`OperationCoordinator` 是**单槽全局租约**，而
    `beginWaiting(..., timeout: 30)` 会**先等满 30 秒**再返回 nil
    ⇒ 用户点完「删除」界面**毫无反应地卡半分钟**，然后**依然什么都没发生** ✗✗。
    而真机上「租约被占着」是常态（续签 / 安装 / Seal 自续签事务都可能占着它）。
  - **判据**：**`delete` 的每一条失败出口都必须设置 `alertFailure` ＋ 写日志码**
    （`SEAL-APP-004` 租约被占用 / `SEAL-APP-005` 存储未就绪）；成功也要留痕
    （`SEAL-APP-006`），因为用户报「我的 App 不见了」时要能区分
    「**他自己删的**」与「**对账删的**」（`SEAL-RECONCILE-007`）。
  - **同族自查（可复用）**：任何「返回 `Bool` 但调用点写 `_ = await`」的**破坏性操作**
    都有这个形态 ⇒ 先 `grep -rn "_ = await" Seal/` 把调用点列出来，逐个问
    「它失败时用户看得见吗」。守卫 **R76** 钉住这一族（退回一行式写法就报红）。
  - ⚠️ **别把它当成「加个弹窗」就完事**：提示必须说清**是谁占着租约**
    （`operationCoordinator?.activeLease?.kind.title`，如「正在续签」），
    否则用户不知道该等什么，只会反复点。

- 🔴 **「一次什么都没做的失败」不配用模态窗；「破坏性操作」必须留痕到「删了谁」**（2026-09-22，构建 201 真机日志）。
  两件事同源：**弹窗的判据仍然是错的，而删除动作本身没有审计线索**。
  - **证据一：判据闸门生效了，但闸门本身错了。** 构建 201 的 7 分钟日志里 `-003` 出现 6 次 ——
    4 次「尝试 3 次，末次耗时 0.0 秒」是 **info 级**，2 次「尝试 1 次，末次耗时 15.0/15.1 秒」是
    **warning 级** ⇒ 分级与设计完全一致 ✓（R73 的判据真的生效了）。
    **但「撞满超时 = 会话已死」被当场证伪**：12:46:59 超时（15.0 秒）→ 12:47:37 通道检测又「正常」；
    12:47:51 超时（15.1 秒）→ 12:47:55 对账就正常跑完了（还删了 1 条）
    ⇒ **15 秒超时是「会话暂时卡住」，不是死** ✗ ⇒ 拿它当弹窗判据，7 分钟里照样产生 2 次假警报 ✗。
  - **证据二：删了谁查不出来。** 日志里有 `-005 探测 2 条，删除 1 条`，而探测的那 2 条正是用户
    **9 分钟前与 1.5 分钟前刚刚装成功**的 Lanerc 与 LiveContainer ⇒ 到底是「用户自己卸了一个」
    还是「通道答错了」，**日志里一个字都答不出来** ✗ —— 而这条路径是**破坏性**的。
  - **判据**：① **模态窗只留给「用户必须马上知道、否则会做错事」的失败**；「本轮什么都没改」的
    失败**只写日志**（级别仍按失败形态分，因为级别不影响是否打断用户）。
    ② **破坏性操作逐条留痕**：App 名 ＋ Bundle ID ＋ 设备的回答 ＋ **触发来源**
    （下拉刷新 / 启动或回到前台）。**验收句：拿着这条日志，能不能说出「删了谁、为什么删、谁触发的」** ✓。
  - ⚠️ **耗时判据也会被证伪** ⇒ 耗时**只配用来分日志级别**，不配当「要不要打断用户」的判据。
  - 🔴 **阳性对照只覆盖那个折叠陷阱的「全局」形态**：它证明通道整体是通的，
    证明不了「它对**这一个** Bundle ID 的否定答案是对的」（底层把 lookup 的 `Err` 与
    「没查到」折成同一个空指针 ⇒ **单条**查询失败同样伪装成「没装」）。
    ⇒ 删除前必须**再问一次**，两次都是「未安装」才真的删
    （判据 = 纯函数 `InstalledAppReconcilePolicy.confirmedRemoval`，＋单测）；
    不一致就**中止整轮**。**这是「一条查询结果既决定删不删、又只问一次」的最后一层漏洞。**
  - ⚠️ **删掉一段实现时，要同时清掉钉它的断言「和」变异锚点**：只清断言会让守卫报
    `Mutation anchor missing`，而那条报错指向的是**锚点**、不指向被删的实现，
    很容易被误读成「锚点写错了」✗（本轮实际踩到，白跑一轮 4 分钟守卫）。
  - ⚠️ **断言用的 `section_or_empty` 起点要覆盖「它要断的那个赋值」**：本轮 R74 的
    `deletion_log` 起点原本取的是**日志文案**那一行，而 `let label = "\(entry.app.name)（\(deletedBundleID)）"`
    定义在它**之前** ⇒ 区段里只有 `\(label)`，看不到 `entry.app.name` ⇒ 断言**假红** ✗。
    **判据：区段起点 = 变异锚点所在那一行**（锚点改的就是 `let label = ` 那行，区段不覆盖它等于没在守）。
...
### 2026-09-22 · 「续签和待签名删不掉」：静默失败 ＋ 调用点丢弃返回值（构建 199 真机日志）

**现象**：用户装了构建 **199**（`Seal-log(3) (1).txt`，819 行，窗口 11:20:07–13:07:48，
表头 `构建 1.2.1 (199)`）并报三件事：① **续签删不掉** ② **待签名删不掉** ③ 手机系统照片闪退。
本条目只处理 ②（①③ 见下方「未解决 / 已证伪」）。

**定位过程（关键是那个「零」）**

1. 先把全部日志码计数：`SEAL-STAGE-001`×54、`SEAL-RECONCILE-003`×45、`SEAL-INSTALL-708`×40、
   `SEAL-RECONCILE-005`×37、`SEAL-PROFILE-320`×30、`SEAL-RECONCILE-001`×29、`SEAL-INSTALL-701`×18 …
   —— **删除相关的码一个都没有**（`SEAL-APP-*` 在源码里当时**根本不存在**）✗。
2. ⇒ 结论不是「删除失败了」，而是「**这条路径从不留痕**」—— 与技能里那条
   「某状态下什么都不做的分支，要问一句『那谁来推进这件事』」同族，
   这次要问的是「**失败时日志里长什么样**」。
3. 读源码即定位：`AppsViewModel.delete(_:)` 开头**两处** `guard ... else { return false }`，
   而调用点 `AppsViewRoot.swift:121` 是 `Task { _ = await viewModel.delete(app) }`
   —— **返回值被丢掉了** ⇒ **没有任何一层会告诉用户「没删成」** ✗。

**根因**

| 出口 | 条件 | 原来的行为 | 用户感受 |
|---|---|---|---|
| ① | `appStore` / `fileStore` 为空 | `return false`，无日志无弹窗 | 点了没反应 |
| ② | 操作租约被占用（**真机常态**） | `return false`，无日志无弹窗 | **卡 30 秒**然后没反应 |

② 的 30 秒来自 `OperationCoordinator`：它是**单槽全局租约**，而
`beginWaiting(..., timeout: 30)` 会**先等满 30 秒**再返回 nil。
而真机上续签 / 安装 / Seal 自续签事务都可能占着这个租约 —— 用户报的「续签和待签名删不掉」
正好是**续签刚跑完**的场景，与代码完全对上。

**修复**（`Seal/Features/Apps/AppsViewModel.swift`）

- 两处失败出口都**设置 `alertFailure` ＋ 写日志**：`SEAL-APP-005`（存储未就绪，error）、
  `SEAL-APP-004`（租约被占用，warning）。
- 提示里**说出是谁占着**：读 `operationCoordinator?.activeLease?.kind.title`
  （如「正在续签」），否则用户不知道该等什么，只会反复点。
- 文案抽成**纯函数** `AppsViewModel.deleteBlockedFailure(activeOperationTitle:)`
  —— 它的错法不崩、不编译失败，只在真机上表现为「点了没反应」，只能靠单测钉住。
- 成功也留痕：`SEAL-APP-006`（`已移除应用记录：<App 名>（<Bundle ID>）`），
  用来区分「**他自己删的**」与「**对账删的**」（`SEAL-RECONCILE-007`）。

**涉及文件**：`Seal/Features/Apps/AppsViewModel.swift`、
`SealTests/Apps/AppsViewModelDeleteFailureTests.swift`（新，2 条）、
`Scripts/verify-release-safety.py`（**R76**：7 条断言 ＋ 5 个变异锚点）、
`docs/qa/log-code-index.md`（新开「删除 App」一节）、`AGENTS.md` §4、本文件。

**验证状态**：⚠️ **待真机**（构建 203）。
验收判据：点「删除」而 App 还在时，日志里**一定能找到** `SEAL-APP-004` / `-005` 之一，
且界面给出弹窗；两者都没有却删不掉 ⇒ 说明点按根本没到 `delete(_:)`（另一个问题）。

**未解决 / 已证伪（同一份日志）**

- ✅ **续签失败（①）已定位：不是签名问题，是设备连不上** —— 见「常犯坑位」第 1 条。
  一句话：35 次通道检测失败的首个错误**全部**是「设备未响应」×24 /「LocalDevVPN 未就绪」×11，
  而签名链路（证书复用／描述文件核验／重签／打包／产物核验）**全程成功**
  ⇒ 根因是 **LocalDevVPN 隧道**，与 iOS 16 配对卡住的是同一件事。
  **本轮不需要改续签代码** —— 已在操作单里请用户确认 VPN 是否真的连上。
- ✅ **照片闪退（③）再次被结构性证伪**：`project.yml` 只有 `NSPhotoLibraryAddUsageDescription`
  （**add-only** ⇒ 读不了也删不了相册）；全仓 `DCIM` / `/var/mobile` / `PhotoData` /
  `Photos.sqlite` **零命中**；碰相册的两处代码（`AppSigningSheet` 的选图标、
  `SealCommunityView` 的存赞赏码）都**不在**刷新/对账链路上。
  ⇒ Seal 结构上不可能删相册。**待查方向**：设备剩余存储（Seal 的 `Apps/<UUID>/` 会累积
  历次签名的 IPA，**没有任何清理策略**）—— 需要真机上「设置 → 存储」的数字才能下结论。

### 2026-09-22 · 对账路径**整体不再弹窗** ＋ 删除必须留痕到「删了谁」（构建 201 真机日志）

**现象**：用户装了构建 201 并导出真机日志（`Seal-log(31).txt`，183 行，窗口 12:40:35–12:47:55，
表头 `构建 1.2.1 (201)`）。日志把上一轮的判据**当场证伪**，并暴露出一个此前没注意到的
**破坏性操作不可审计**的问题。

**三条硬证据**

1. **判据闸门本身是对的**：`SEAL-RECONCILE-003` 出现 6 次 ——
   4 次「尝试 3 次，末次耗时 0.0 秒」是 **info 级**，
   2 次（12:46:59 / 12:47:51）「尝试 1 次，末次耗时 15.0 / 15.1 秒」是 **warning 级**
   ⇒ 分级与设计完全一致 ✓（R73 的判据真的生效了）。
2. 🔴 **但「撞满超时 = 会话已死」被证伪**：12:46:59 超时（15.0 秒）→ **12:47:37 通道检测又「正常」**；
   12:47:51 超时（15.1 秒）→ **12:47:55 对账就正常跑完了**（还删了 1 条）
   ⇒ **15 秒超时是「会话暂时卡住」，不是死** ✗ ⇒ 拿它当弹窗判据，7 分钟里照样 2 次假警报 ✗。
3. 🔴 **删除动作查不出「删了谁」**：日志里有 `SEAL-RECONCILE-005 探测 2 条，删除 1 条`，
   而探测的那 2 条正是用户 **12:42:27 装的 Lanerc 与 12:46:30 装的 LiveContainer**
   （都是刚装成功的）⇒ 到底是「用户自己卸了一个」还是「通道答错了」，**日志里答不出来** ✗。

**根因**

- 判据层：`InstalledAppProbeFailurePolicy` 把「耗时」当成「会话是否已死」的代理指标，
  而真机证明**同一条通道在 15 秒超时之后 40 秒内就会恢复**（`-003` 超时 → 通道检测正常）。
  ⇒ 耗时**只能**用来分日志级别（那个用途是成立的、也是用户可判读的），
  **不能**用来决定「要不要打断用户」。
- 审计层：删除只在**汇总行**报计数（`探测 N 条，删除 M 条`）。这条路径是**破坏性**的，
  而汇总行既不能确认它删对了，也不能在删错时知道丢了什么。

**修复**

- 🔴 **对账路径不再弹任何模态窗**（`reconcileInstalledAppsWithDevice` 里现在**零** `alertFailure`）：
  - 阳性对照中止分支（`-003`）：只写日志，级别 `deservesAttention ? .warning : .info`；
    判据只有一句 —— **一次「什么都没做」的失败，不配打断用户**（本轮一条记录都没删，
    列表也已从本地库刷新过 ⇒ 用户没有需要立刻处理的后果）。
  - 逐条查询失败分支（`-004`）：**同样不弹**（原先刻意保留弹窗，真机日志证明这类坏掉
    也会自己恢复）⇒ 之前那个「刻意不对称」被真机数据推翻了，注释已同步改写。
  - 「VPN 没开」这个信息**有自己的入口**：设置里的通道检测会写 `SEAL-INSTALL-706b`
    并给出「请确认…是否打开 LocalDevVPN」的完整说明（同一份日志里就有）。
  - 删掉 `reconcileAbortFailure(detail:)` 工厂（没有调用方了）。
- 🔴 **删除逐条留痕**：新增 `SEAL-RECONCILE-007` ——
  `<App 名>（<Bundle ID>）—— 设备答「<回答>」且阳性对照已通过、否定答案复核一致；触发：…`；
  `-005` 汇总行追加 `（删除：<App 名>（<Bundle ID>）、…）`。
- 🔴 **删除前「再问一次」**：新增纯函数
  `InstalledAppReconcilePolicy.confirmedRemoval(first:confirmation:)`（＋单测），
  调用点改成 `guard InstalledAppReconcilePolicy.confirmedRemoval(…)`；
  两次不一致就**中止整轮**并写 `SEAL-RECONCILE-008`（日志里带两次分别答了什么）。
  理由：**阳性对照只覆盖那个「把 lookup 的 `Err` 与『没查到』折成同一个空指针」的陷阱的
  全局形态**（通道整体不通），覆盖不了**单条**形态 ⇒ 一次否定答案就删仍然可能删错。
- **触发来源写进日志**：新增 `triggerText(_:)`，4 个调用点（`-003` / `-004` / `-005` / `-007`）
  统一带 `触发：下拉刷新` 或 `触发：启动或回到前台` ——
  构建 201 的日志里有 2 条超时中止，正因为缺这一项而**无法判断用户当时是否在下拉刷新**。

**守卫与测试**

- 守卫新增 **R74**（7 条断言 ＋ 6 个变异锚点），其中一条钉「对账区段里不许出现 `alertFailure`」，
  一条钉「删除日志必须同时带应用名 ＋ Bundle ID ＋ 设备的回答」，
  一条钉「`-005` 汇总行必须列出删掉的 App 名字」，一条钉「5 个调用点都带触发来源」。
- 守卫新增 **R75**（8 条断言 ＋ 6 个变异锚点），钉「删除前必须再问一次、且必须在 `await delete(` 之前」、
  「两次都必须是否定答案（判据走纯函数）」、
  「不一致必须**中止整轮**（不是 `continue`）」、「日志要写出两次的答案」、
  「纯函数本体与单测都必须存在」。
- 移除 3 条 R72 断言（弹窗 `recovery: "知道了"` / 「本轮未改动任何记录」/ 工厂调用计数）
  **和 2 个 R72 变异锚点** —— 它们钉的实现已被删除。
  ⚠️ 本轮**踩到**：只清了断言、忘了清锚点 ⇒ 守卫报 `Mutation anchor missing`，
  白跑一轮 4 分钟守卫；报错指向锚点而不指向被删的实现，很容易误读成「锚点写错了」✗。
- R73 的「先定性、再决定」顺序断言改为钉 `level: deservesAttention ? .warning : .info` 那一行
  （原来的 `if userInitiated, deservesAttention {` 已不存在）。
- 守卫规模：**532 / 280 → 544 / 290**（净 ＋12 断言 / ＋10 锚点；含移除 3 条 R72 断言与 2 个 R72 锚点）。
- ⚠️ 本轮 R74 断言**假红过一次**：`deletion_log` 的 `section_or_empty` 起点取的是**日志文案**
  那一行，而 `let label = "\(entry.app.name)（\(deletedBundleID)）"` 定义在它**之前**
  ⇒ 区段里只有 `\(label)` ⇒ 断言看不到 `entry.app.name`。
  **判据：区段起点 = 变异锚点所在那一行**（锚点改的就是 `let label = `，区段不覆盖它等于没在守）。

**涉及文件**：`Seal/Features/Apps/AppsViewModel.swift`、`Scripts/verify-release-safety.py`、
`AGENTS.md` §4、`docs/qa/log-code-index.md`、本文件。

**验证状态**：⏳ **待真机** —— 判据是「下拉刷新**永不**出现弹窗（无论快慢）」＋
「万一有删除，`-007` 能说出删了谁、设备怎么答的、谁触发的」。
⚠️ 构建 201 的日志里 `SEAL-NOTIFY-002a` **一次都没出现**（构建 200 的修复成立 ✓）。
...
> ⚠️ **后续（2026-09-22，同日）：这一版的判据被真机日志证伪了。**
> 构建 201 的日志里，2 次「撞满超时（15.0 / 15.1 秒）」之后通道都在 40 秒内恢复
> ⇒ **15 秒超时 ≠ 会话已死**，拿它当弹窗判据照样产生假警报。
> 最终结论是**对账路径整体不弹窗**（见上一条「构建 201 真机日志」）——
> 本条目里「只有 `.sessionDead` 才弹」「`-004` 刻意保留弹窗」这两处**都已被推翻**，
> 保留原文仅为记录判据演进过程。
